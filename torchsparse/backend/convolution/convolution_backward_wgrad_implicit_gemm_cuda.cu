...
__global__ void __launch_bounds__(32) conv_backward_cuda_setting1_mode0_bf16bf16f32(int M_fwd, int64_t K_original, int64_t N, int kernel_volume, int split_k_iters, __nv_bfloat16*__restrict__ A, __nv_bfloat16*__restrict__ B, int *__restrict__ out_in_map, __nv_bfloat16*__restrict__ C)
{
  ...
  // __nv_bfloat16*A_ptr = A + ((threadIdx.y * 256 % 16) % K_original) + ((threadIdx.x * 8 % 16) % K_original) + ((blockIdx_y / j_factors1 * 16) % K_original);
  ...
  // __nv_bfloat16*C_ptr = cur_C + blockIdx_x / 1 * 108 * N / 16 * 256 + blockIdx_y / j_factors1 * 1 * N / 16 * 256 + (threadIdx.y % 1) * 1 * N / 16 * 256 + (blockIdx_x % 1) * j_factors1 * 16 + (blockIdx_y % j_factors1) * 16 + threadIdx.y / 1 * 16 + (threadIdx.x % 4) * 2 + (threadIdx.x / 4) * N;
...
__global__ void __launch_bounds__(64) conv_backward_cuda_setting2_mode0_bf16bf16f32(int M_fwd, int64_t K_original, int64_t N, int kernel_volume, int split_k_iters, __nv_bfloat16*__restrict__ A, __nv_bfloat16*__restrict__ B, int *__restrict__ out_in_map, __nv_bfloat16*__restrict__ C)
{
    ...
    bool allow_tf32, bool allow_bf16)
{
  ...
  if (is_bf16)
  {
    ...
    else
    {
      ...
      // conv_backward_cuda_setting1_mode0_bf16bf16f32<<<num_blocks, threads_per_block>>>(
      //     _kernel.size(0), num_in_channels, num_out_channels, kernel_volume, split_k_iters, in_feats, kernel, out_in_map, out_feats);      
