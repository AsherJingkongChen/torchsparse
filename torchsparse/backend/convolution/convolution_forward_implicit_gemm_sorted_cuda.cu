...
__global__ void __launch_bounds__(128) conv_forward_cuda_setting3_mode1_bf16bf16f32(int64_t M, int64_t K_original, int64_t N, int kernel_volume, int split_mask_len, int reduced_mask_len, int reorder_loc_len, __nv_bfloat16*__restrict__ A, __nv_bfloat16*__restrict__ B, int *__restrict__ reduced_mask, int *__restrict__ out_in_map, int *__restrict__ reorder_loc, __nv_bfloat16*__restrict__ C)
{
  int K_implicit = K_original * kernel_volume;
  float C_warp[64];
  __shared__ __nv_bfloat16 A_shared[5120];
    ...
    bool allow_tf32, bool allow_bf16)
{
  ...
  if (is_bf16)
  {
    ...
    else
    {
      ...
      // threadIdx.y: i_factors[2] * j_factors[2]
      dim3 threads_per_block(32, 2);
      // conv_forward_cuda_setting1_mode1_bf16bf16f32<<<num_blocks, threads_per_block>>>(
